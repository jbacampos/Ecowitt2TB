...
struct WeatherData {
   ...
   float wh40h_battery_voltage;
   float ws90_battery_voltage;
};
...
const size_t SEGMENT_MAX_SIZE = SEGMENT_HEADER_SIZE + telemetry_queue::MAX_RECORDS_PER_SEGMENT * telemetry_queue::RECORD_SIZE + SEGMENT_FOOTER_SIZE;
...
float getArrayVoltage(JsonArray array) {
   if (array.isNull())
      return NAN;
   for (JsonObject item : array) {
      if (item.isNull())
      ...
      if (!id || strcmp(id, "0x13") != 0)
         continue;
      if (!item.containsKey("voltage") || item["voltage"].isNull())
         return NAN;
      if (item["voltage"].is<float>() || item["voltage"].is<double>() || item["voltage"].is<int>()) {
         return item["voltage"].as<float>();
      }
      const char *strVal = item["voltage"];
      if (strVal && strVal[0] != '\0')
         return String(strVal).toFloat();
      return NAN;
   }
   return NAN;
}
...
uint64_t currentTimestampMs() { return static_cast<uint64_t>(time(nullptr)) * 1000ULL + (millis() % 1000UL); }
...
uint64_t parseGatewayTimestamp(const String &timestampText) {
   ...
   int valuesRead = sscanf(timestampText.c_str(), "%d%*[-/]%d%*[-/]%d %d:%d:%d", &year, &month, &day, &hour, &minute, &second);

   if (valuesRead != 6 || year < 2024 || month < 1 || month > 12 || day < 1 || day > 31 || hour > 23 || minute > 59 || second > 59)
      return 0;
...
void connectMQTT() {
   ...
   if (lastAttempt != 0 && millis() - lastAttempt < MQTT_RETRY_INTERVAL)
      return;
...
bool readEcowitt(WeatherData &data) {
   ...
   String url = String("http://") + ECOWITT_HOST + ":" + ECOWITT_PORT + "/get_livedata_info?";
   ...
   data.temperature_out = getValue(common, "0x02");

   data.feels_like_out = getValue(common, "3");

   data.humidity_out = getValue(common, "0x07");

   data.dew_point_out = getValue(common, "0x03");

   data.wind_speed = getValue(common, "0x0B");

   data.wind_gust = getValue(common, "0x0C");

   data.wind_direction = getValue(common, "0x0A");

   data.wind_direction_10min = getValue(common, "0x6D");

   data.solar_radiation = getValue(common, "0x15");
   ...
   if (!wh25.isNull() && wh25.size() > 0) {
      ...
      data.temperature_in = String((const char *)indoor["intemp"]).toFloat();

      data.humidity_in = String((const char *)indoor["inhumi"]).toFloat();

      data.pressure_relative = String((const char *)indoor["rel"]).toFloat();
   } else {
   ...
   // --------------------------------------------------------
   // rain (WH40H) e piezoRain (WS90)
   // --------------------------------------------------------

   JsonArray rain = doc["rain"].as<JsonArray>();
   JsonArray piezoRain = doc["piezoRain"].as<JsonArray>();

   // Chuva proveniente do pluviômetro tradicional WH40H (rain[])
   data.rain_event = getValue(rain, "0x0D");

   data.rain_rate = getValue(rain, "0x0E");

   // 0x7D = novo Rain Hour
   data.rain_hour = getValue(rain, "0x7D");

   data.rain_day = getValue(rain, "0x10");

   data.rain_week = getValue(rain, "0x11");

   data.rain_month = getValue(rain, "0x12");

   data.rain_year = getValue(rain, "0x13");

   // 0x14 = Rain Totals
   data.rain_total = getValue(rain, "0x14");

   // Baterias
   data.wh40h_battery_voltage = getArrayVoltage(rain);
   data.ws90_battery_voltage = getArrayVoltage(piezoRain);
...
void printWeatherData(const WeatherData &d) {
   ...
   Serial.printf("Chuva [WH40H] evento : %.2f\n", d.rain_event);

   Serial.printf("Chuva [WH40H] taxa   : %.2f\n", d.rain_rate);

   Serial.printf("Chuva [WH40H] hora   : %.2f\n", d.rain_hour);

   Serial.printf("Chuva [WH40H] dia    : %.2f\n", d.rain_day);

   Serial.printf("Chuva [WH40H] semana : %.2f\n", d.rain_week);

   Serial.printf("Chuva [WH40H] mes    : %.2f\n", d.rain_month);

   Serial.printf("Chuva [WH40H] ano    : %.2f\n", d.rain_year);

   Serial.printf("Chuva [WH40H] total  : %.2f\n", d.rain_total);

   Serial.printf("Bateria WH40H       : %.2f V\n", d.wh40h_battery_voltage);

   Serial.printf("Bateria WS90        : %.2f V\n", d.ws90_battery_voltage);
...
void makeTelemetryPayload(const WeatherData &d, uint64_t collectedAt, String &payload) {
   JsonDocument doc;
   ...
   values["wh40h_battery_voltage"] = d.wh40h_battery_voltage;
   values["ws90_battery_voltage"] = d.ws90_battery_voltage;
...
telemetry_queue::Values weatherValues(const WeatherData &d) {
   telemetry_queue::Values values = { { d.temperature_out,
                                        d.feels_like_out,
                                        d.humidity_out,
                                        d.dew_point_out,
                                        d.pressure_relative,
...
WeatherData valuesWeather(const telemetry_queue::Values &v, uint64_t timestamp) {
   ...
   d.temperature_out = v.value[0];
   d.feels_like_out = v.value[1];
   d.humidity_out = v.value[2];
   d.dew_point_out = v.value[3];
   d.pressure_relative = v.value[4];
   d.wind_speed = v.value[5];
   d.wind_gust = v.value[6];
   d.wind_direction = v.value[7];
   d.wind_direction_10min = v.value[8];
   d.solar_radiation = v.value[9];
   d.temperature_in = v.value[10];
   d.humidity_in = v.value[11];
   d.rain_rate = v.value[12];
   d.rain_event = v.value[13];
   d.rain_hour = v.value[14];
   d.rain_day = v.value[15];
   d.rain_week = v.value[16];
   d.rain_month = v.value[17];
   d.rain_year = v.value[18];
   d.rain_total = v.value[19];
   d.wh40h_battery_voltage = v.value[20];
   d.ws90_battery_voltage = v.value[21];
   return d;
}

bool writeExact(File &file, const uint8_t *data, size_t size) { return file.write(data, size) == size; }

bool readExact(File &file, uint8_t *data, size_t size) { return file.read(data, size) == static_cast<int>(size); }
...
bool readSequenceMeta(int slot, uint32_t &generation, uint32_t &segmentLimit, uint32_t &recordLimit) {
   File file = LittleFS.open(SEQUENCE_META_FILES[slot], "r");
   uint8_t data[SEQUENCE_META_SIZE];
   bool valid = file && file.size() == sizeof(data) && readExact(file, data, sizeof(data));
   if (file)
      file.close();
   if (!valid || telemetry_queue::get32(data) != SEQUENCE_META_MAGIC || telemetry_queue::get32(data + 16) != telemetry_queue::crc32(data, 16))
      return false;
...
bool writeSequenceMeta(uint32_t segmentLimit, uint32_t recordLimit) {
   ...
   bool verified = ok && readSequenceMeta(target, checkGeneration, checkSegments, checkRecords) && checkGeneration == generation &&
                   checkSegments == segmentLimit && checkRecords == recordLimit;
   if (!verified)
      return false;
...
bool reserveSequenceRanges(uint32_t segmentFloor, uint32_t recordFloor) {
   const uint32_t segmentBase = max(reservedSegmentLimit, max(segmentFloor, nextSegmentSequence));
   const uint32_t recordBase = max(reservedRecordLimit, max(recordFloor, nextRecordSequence));
   if (segmentBase > 99999999UL - SEGMENT_SEQUENCE_RESERVATION || recordBase > UINT32_MAX - RECORD_SEQUENCE_RESERVATION - 1) {
      Serial.println("Espaço de sequências esgotado; fila desativada.");
      return false;
   }
   return writeSequenceMeta(segmentBase + SEGMENT_SEQUENCE_RESERVATION, recordBase + RECORD_SEQUENCE_RESERVATION);
}

bool initializeSequenceMeta(uint32_t segmentFloor, uint32_t recordFloor, uint32_t &segmentResume, uint32_t &recordResume) {
   const bool exists[2] = { LittleFS.exists(SEQUENCE_META_FILES[0]), LittleFS.exists(SEQUENCE_META_FILES[1]) };
   uint32_t generation[2] = {}, segmentLimit[2] = {}, recordLimit[2] = {};
   bool valid[2] = { readSequenceMeta(0, generation[0], segmentLimit[0], recordLimit[0]), readSequenceMeta(1, generation[1], segmentLimit[1], recordLimit[1]) };
   sequenceMetaSlot = -1;
   ...
   if (valid[0] || valid[1]) {
      sequenceMetaSlot = valid[1] && (!valid[0] || static_cast<int32_t>(generation[1] - generation[0]) > 0) ? 1 : 0;
   ...
   if (!reserveSequenceRanges(segmentFloor, recordFloor))
      return false;
   if (hadValidMeta) {
...
bool createSegment(uint32_t sequence, String &path) {
   size_t total, used;
   if (!getFilesystemSpace(total, used) || total <= used || total - used <= FILESYSTEM_SAFETY_MARGIN + SEGMENT_MAX_SIZE) {
...
bool closeSegment(SegmentInfo &segment) {
   if (segment.closed)
      return true;
   const size_t recordsEnd = SEGMENT_HEADER_SIZE + segment.count * telemetry_queue::RECORD_SIZE;
   ...
   if (check && check.size() == SEGMENT_HEADER_SIZE + segment.count * telemetry_queue::RECORD_SIZE + SEGMENT_FOOTER_SIZE &&
       check.seek(check.size() - SEGMENT_FOOTER_SIZE)) {
      uint8_t stored[SEGMENT_FOOTER_SIZE];
      footerValid = readExact(check, stored, sizeof(stored)) && telemetry_queue::get32(stored) == FOOTER_MAGIC &&
                    telemetry_queue::get32(stored + 4) == segment.count && telemetry_queue::get32(stored + 8) == telemetry_queue::crc32(stored, 8);
   }
   if (check)
      check.close();
   if (!footerValid)
...
bool appendWeather(const WeatherData &data) {
   if (!queueReady)
      return false;
   if (nextRecordSequence >= reservedRecordLimit || nextSegmentSequence >= reservedSegmentLimit) {
      SegmentInfo *current = nullptr;
      for (auto &segment : segments)
         if (segment.valid && !segment.closed)
            current = &segment;
      if (current && nextRecordSequence >= reservedRecordLimit && current->count && !closeSegment(*current))
         return false;
   ...
   if (!active && !segments.empty()) {
      ...
      if (nextRecordSequence < expectedFirst)
         return false;
   }
   if (!active) {
      String path;
      if (!createSegment(nextSegmentSequence, path))
         return false;
   ...
   if (!getFilesystemSpace(total, used) || total <= used ||
       total - used <= FILESYSTEM_SAFETY_MARGIN +
                           (active->count == 0
                                ? SEGMENT_MAX_SIZE
                                : telemetry_queue::RECORD_SIZE + (active->count + 1 == telemetry_queue::MAX_RECORDS_PER_SEGMENT ? SEGMENT_FOOTER_SIZE : 0))) {
      Serial.println("LittleFS: margem de segurança impede enfileirar amostra.");
      return false;
   }

   File file = LittleFS.open(active->path, "a");
   if (!file || file.size() != SEGMENT_HEADER_SIZE + active->count * telemetry_queue::RECORD_SIZE) {
      if (file)
         file.close();
   ...
   telemetry_queue::encodeRecord(record, nextRecordSequence, data.collectedAt, weatherValues(data));
   bool ok = writeExact(file, record, sizeof(record));
   file.flush();
   file.close();
   File verify = LittleFS.open(active->path, "r");
   const size_t expectedSize = SEGMENT_HEADER_SIZE + (active->count + 1) * telemetry_queue::RECORD_SIZE;
   ...
   ++active->count;
   ++nextRecordSequence;
   if (active->count == telemetry_queue::MAX_RECORDS_PER_SEGMENT && !closeSegment(*active)) {
      if (active->closed)
         active->recoverableTail = false;
...
bool inspectSegment(SegmentInfo &segment, uint32_t &lastRecordSequence, bool &haveRecordSequence) {
   File file = LittleFS.open(segment.path, "r");
   if (!file)
      return false;
   uint8_t header[SEGMENT_HEADER_SIZE];
   bool valid = readExact(file, header, sizeof(header)) && telemetry_queue::get32(header) == SEGMENT_MAGIC && header[4] == SEGMENT_VERSION &&
                header[5] == SEGMENT_HEADER_SIZE && telemetry_queue::get32(header + 8) == segment.sequence &&
                telemetry_queue::get32(header + 20) == telemetry_queue::crc32(header, 20);
   uint32_t expected = 0;
   uint16_t count = 0;
   while (valid && count < telemetry_queue::MAX_RECORDS_PER_SEGMENT && file.size() - file.position() >= telemetry_queue::RECORD_SIZE) {
      uint8_t record[telemetry_queue::RECORD_SIZE];
      uint64_t timestamp;
   ...
   if (valid && tail == SEGMENT_FOOTER_SIZE) {
      uint8_t footer[SEGMENT_FOOTER_SIZE];
      closed = readExact(file, footer, sizeof(footer)) && telemetry_queue::get32(footer) == FOOTER_MAGIC && telemetry_queue::get32(footer + 4) == count &&
               telemetry_queue::get32(footer + 8) == telemetry_queue::crc32(footer, 8);
      valid = closed;
   } else if (count == telemetry_queue::MAX_RECORDS_PER_SEGMENT && tail < SEGMENT_FOOTER_SIZE) {
...
bool recoverQueue() {
   ...
   while (dir.next()) {
      uint32_t sequence;
      if (!parseSegmentName(dir.fileName(), sequence))
         continue;
      if (sequence > highestSegmentSequence)
         highestSegmentSequence = sequence;
   ...
   std::sort(segments.begin(), segments.end(), [](const SegmentInfo &a, const SegmentInfo &b) { return a.sequence < b.sequence; });
   ...
   for (size_t i = 0; i < segments.size(); ++i) {
      ...
      if (!inspectSegment(segments[i], lastRecord, haveRecord)) {
         Serial.print("Segmento inválido preservado: ");
   ...
   if (!segments.empty() && segments.back().recoverableTail) {
      ...
      const size_t validEnd = SEGMENT_HEADER_SIZE + segments.back().count * telemetry_queue::RECORD_SIZE;
   ...
   if (!initializeSequenceMeta(nextSegmentSequence, nextRecordSequence, segmentResume, recordResume))
      return false;
   if (segmentResume > 99999999UL || recordResume == UINT32_MAX)
      return false;
   if (!segments.empty() && !segments.back().closed) {
      if (!closeSegment(segments.back()))
         return false;
   }
   ...
   if (getFilesystemSpace(total, used)) {
      Serial.printf("LittleFS total=%u usado=%u livre=%u\n", static_cast<unsigned>(total), static_cast<unsigned>(used), static_cast<unsigned>(total - used));
   }
   return true;
}

bool readQueuedRecord(const SegmentInfo &segment, uint16_t index, uint64_t &timestamp, telemetry_queue::Values &values) {
   File file = LittleFS.open(segment.path, "r");
   if (!file || !file.seek(SEGMENT_HEADER_SIZE + static_cast<size_t>(index) * telemetry_queue::RECORD_SIZE)) {
      if (file)
         file.close();
      return false;
   }
   uint8_t bytes[telemetry_queue::RECORD_SIZE];
   bool ok = readExact(file, bytes, sizeof(bytes)) && telemetry_queue::decodeRecord(bytes, telemetry_queue::recordSequence(bytes), timestamp, values) &&
...
enum QueueFsTestPhase : uint32_t { QUEUE_FS_TEST_IDLE = 0, QUEUE_FS_TEST_EXPECT_SEGMENT = 1, QUEUE_FS_TEST_EXPECT_EMPTY = 2 };
...
bool writeQueueFsTestState(const QueueFsTestState &state) {
   ...
   if (verify)
      verify.close();
   if (!ok || !valid || memcmp(bytes, check, sizeof(bytes)) != 0)
      return false;
...
bool loadQueueFsTestState() {
   if (!LittleFS.exists(QUEUE_FS_TEST_STATE))
      return true;
   ...
   if (file)
      file.close();
   if (!valid || telemetry_queue::get32(bytes) != QUEUE_FS_TEST_MAGIC || telemetry_queue::get32(bytes + 16) != telemetry_queue::crc32(bytes, 16))
      return false;
...
bool queueIsEmptyForTest() { return segments.empty() && !queueFsTestSegmentExists() && !LittleFS.exists(PENDING_TELEMETRY_FILE); }

bool verifyTestRecords(const SegmentInfo &segment, uint32_t firstRecord) {
   if (!segment.valid || !segment.closed || segment.count != 3 || segment.firstRecordSequence != firstRecord || nextRecordSequence <= firstRecord + 2)
      return false;
   for (uint16_t i = 0; i < 3; ++i) {
      uint64_t timestamp;
      telemetry_queue::Values values;
      if (!readQueuedRecord(segment, i, timestamp, values) || timestamp != 1700000000000ULL + (i + 1) * 1000 || values.value[0] != 21.5f + i + 1 ||
          values.value[2] != 57.0f)
         return false;
   }
   return true;
}
...
bool printQueueStorageMetrics() {
   ...
   if (!recordCount || usedBytes < queueBytes || totalBytes <= usedBytes - queueBytes + FILESYSTEM_SAFETY_MARGIN) {
   ...
   const size_t queueCapacityBytes = totalBytes - (usedBytes - queueBytes) - FILESYSTEM_SAFETY_MARGIN;
...
bool captureSequenceMetadataBeforeRecovery() {
   ...
   for (int slot = 0; slot < 2; ++slot) {
      queueFsTestMetaBeforeRecovery.valid[slot] =
          LittleFS.exists(SEQUENCE_META_FILES[slot]) && readSequenceMeta(slot,
   ...
   queueFsTestMetaSnapshotValid = queueFsTestMetaBeforeRecovery.valid[0] && queueFsTestMetaBeforeRecovery.valid[1];
   return queueFsTestMetaSnapshotValid;
}

bool verifyRecoveredSequenceMetadata(uint32_t previousSegment, uint32_t previousRecord, uint32_t &expectedSegment, uint32_t &expectedRecord) {
   if (!queueFsTestMetaSnapshotValid || sequenceMetaSlot < 0 || sequenceMetaSlot > 1)
      return false;
   ...
   const uint32_t selectedGeneration = queueFsTestMetaBeforeRecovery.generation[selectedSlot];
   const uint32_t selectedSegmentLimit = queueFsTestMetaBeforeRecovery.segmentLimit[selectedSlot];
   const uint32_t selectedRecordLimit = queueFsTestMetaBeforeRecovery.recordLimit[selectedSlot];
   const uint32_t otherGeneration = queueFsTestMetaBeforeRecovery.generation[sequenceMetaSlot];
   const bool selectedWasLatest = selectedSlot == 1
                                      ? (!queueFsTestMetaBeforeRecovery.valid[0] || static_cast<int32_t>(selectedGeneration - otherGeneration) > 0)
                                      : (!queueFsTestMetaSnapshotValid || static_cast<int32_t>(otherGeneration - selectedGeneration) <= 0);

   if (!queueFsTestMetaBeforeRecovery.valid[selectedSlot] || !selectedWasLatest || sequenceMetaGeneration != selectedGeneration + 1 ||
       reservedSegmentLimit != selectedSegmentLimit + SEGMENT_SEQUENCE_RESERVATION ||
       reservedRecordLimit != selectedRecordLimit + RECORD_SEQUENCE_RESERVATION || nextSegmentSequence != selectedSegmentLimit ||
       nextRecordSequence != selectedRecordLimit || nextSegmentSequence <= previousSegment || nextRecordSequence <= previousRecord)
      return false;
...
bool startQueueFsRebootTest() {
   if (!queueFsTestStateValid || queueFsTestState.phase != QUEUE_FS_TEST_IDLE || !queueIsEmptyForTest()) {
   ...
   if (!reserveSequenceRanges(nextSegmentSequence, nextRecordSequence) || !reserveSequenceRanges(nextSegmentSequence, nextRecordSequence))
      return false;
   ...
   for (uint8_t i = 0; i < 3; ++i) {
      ...
      if (!appendWeather(sample))
         return false;
   }
   if (segments.size() != 1 || segments[0].sequence != firstSegment || segments[0].firstRecordSequence != firstRecord || !closeSegment(segments[0]))
      return false;
   if (!printQueueStorageMetrics())
      return false;
   QueueFsTestState state = { QUEUE_FS_TEST_EXPECT_SEGMENT, firstSegment, firstRecord };
   if (!writeQueueFsTestState(state))
      return false;
...
bool handleQueueFsTestBoot() {
   ...
   if (queueFsTestState.phase == QUEUE_FS_TEST_EXPECT_SEGMENT) {
      if (segments.size() != 1 || segments[0].sequence != queueFsTestState.segmentSequence ||
          !verifyTestRecords(segments[0], queueFsTestState.firstRecordSequence))
         return false;
      Serial.println("TEST: reboot físico confirmado; recoverQueue encontrou segmento e sequências íntegros.");
      if (!removeCompletedSegment(segments[0]))
         return false;
      segments.clear();
      if (!queueIsEmptyForTest())
         return false;
      QueueFsTestState state = { QUEUE_FS_TEST_EXPECT_EMPTY, queueFsTestState.segmentSequence, queueFsTestState.firstRecordSequence };
      if (!writeQueueFsTestState(state))
         return false;
   ...
   if (queueFsTestState.phase == QUEUE_FS_TEST_EXPECT_EMPTY) {
      ...
      if (!queueIsEmptyForTest() ||
          !verifyRecoveredSequenceMetadata(queueFsTestState.segmentSequence, queueFsTestState.firstRecordSequence + 3, expectedSegment, expectedRecord))
         return false;
      ...
      if (!appendWeather(sample) || segments.size() != 1 || segments[0].sequence != expectedSegment || segments[0].firstRecordSequence != expectedRecord ||
          !closeSegment(segments[0]))
         return false;
      ...
      uint64_t timestamp;
      telemetry_queue::Values values;
      if (!readQueuedRecord(segments[0], 0, timestamp, values) || timestamp != sample.collectedAt || values.value[0] != sample.temperature_out)
         return false;
      ...
      if (!removeCompletedSegment(segments[0]))
         return false;
      segments.clear();
      if (!queueIsEmptyForTest() || !LittleFS.remove(QUEUE_FS_TEST_STATE))
         return false;
...
bool processLegacyBatch(size_t &budget) {
   ...
   while (budget && file.available() && millis() - started < REPLAY_BUDGET_MS) {
      ...
      JsonDocument doc;
      if (deserializeJson(doc, record) || !doc["ts"].is<uint64_t>() || !doc["values"].is<JsonObject>()) {
   ...
   bool drained = reachedEnd && !file.available() && legacyPosition == file.size() && budget > 0;
   file.close();
   if (drained) {
...
void publishPendingTelemetry() {
   ...
   while (budget && replaySegment < static_cast<int>(segments.size()) && millis() - started < REPLAY_BUDGET_MS) {
      SegmentInfo &segment = segments[replaySegment];
      if (!segment.valid)
         return;
      if (replayRecord >= segment.count) {
         if (segment.closed) {
            if (!removeCompletedSegment(segment))
               return;
      ...
      if (!publishPayload(payload, timestamp))
         return;
      ++replayRecord;
      --budget;
   }
   ...
   if (replaySegment < static_cast<int>(segments.size())) {
      SegmentInfo &segment = segments[replaySegment];
      if (replayRecord == segment.count && segment.closed && removeCompletedSegment(segment)) {
         segments.erase(segments.begin() + replaySegment);
         replayRecord = 0;
      }
   }
...
bool publishPayload(const String &payload, uint64_t collectedAt) {
   ...
   bool result = mqtt.publish(TB_TOPIC, payload.c_str(), payload.length());
...
bool savePendingPayload(const String &payload) {
   JsonDocument doc;
   if (deserializeJson(doc, payload) || !doc["ts"].is<uint64_t>() || !doc["values"].is<JsonObject>()) {
      Serial.println("Payload não pode ser codificado na fila binária.");
      return false;
   }
   WeatherData data{};
   JsonObject values = doc["values"].as<JsonObject>();
   const char *keys[22] = { "temperature_out",
                            "feels_like_out",
                            "humidity_out",
                            "dew_point_out",
                            "pressure_relative",
                            ...
                            "rain_year",
                            "rain_total",
                            "wh40h_battery_voltage",
                            "ws90_battery_voltage" };
   float decoded[22];
...
void setup() {
   ...
   if (!LittleFS.begin()) {
      ...
      } else if (queueFsTestState.phase == QUEUE_FS_TEST_EXPECT_EMPTY && !captureSequenceMetadataBeforeRecovery()) {
   ...
   configTime(UTC_OFFSET_SECONDS, 0, NTP_SERVER_1, NTP_SERVER_2, NTP_SERVER_3);
...
void loop() {
   ...
   if (Serial.available()) {
      ...
      if (command == 't' || command == 'T') {
         ...
         if (!passed)
            Serial.println("RESULTADO: FAIL ao preparar segmento.");
      } else if (command == 'r' || command == 'R') {
         if (queueFsTestState.phase == QUEUE_FS_TEST_IDLE) {
            Serial.println("Nenhum reboot está pendente.");
         } else if (queueFsTestState.phase == QUEUE_FS_TEST_EXPECT_SEGMENT && segments.size() == 1 && segments[0].closed) {
            Serial.println("AÇÃO EXPLÍCITA CONFIRMADA: pressione agora o botão RESET do ESP8266.");
         } else if (queueFsTestState.phase == QUEUE_FS_TEST_EXPECT_EMPTY && queueIsEmptyForTest()) {
            Serial.println("AÇÃO EXPLÍCITA CONFIRMADA: pressione agora o botão RESET do ESP8266.");
         } else {
